...
from main_granger_extend import test_granger_extend
...
for aggre_delta in range(1, 2):
    for before_length in np.arange(0, 300, 20):
                                    ...
                                    for topk_path in [150]:
                                        for num_sel_node in [3]:
                                            if before_length != 0 or after_length != 0:
                                                for runtime_debug_i in range(5):
                                                    params_list.append({
                                                        'ela': aggre_delta,
                                                        'bef': before_length, 
                                                        'aft': after_length, 
                                                        ...
                                                        'runtime_debug_ind': runtime_debug_i
                                                        })
...
def main():
    ...
    for i, params_dict in enumerate(params_list[0:1]):
    ...
    if len(result_list) > 0:
        with open('granger_extend_runtime_ibm_708.pkl', 'wb') as f:
